...
namespace gambatte
{
	...
	CPU::CPU()
		: mem_(Interrupter(sp, pc, opcode_, prefetched_)), cycleCounter_(0), pc(0x100), sp(0xFFFE), hf1(0xF), hf2(0xF), zf(0), cf(0x100), a(0x01), b(0x00), c(0x13), d(0x00), e(0xD8), h(0x01), l(0x4D), opcode_(0), prefetched_(false), numInterruptAddresses(0), traceCallback_(0)
	{
	}
	...
	{
		return ((hf2 & (hf2_subf | hf2_hcf)) | (cf & 0x100)) >> 4 | (zf & 0xFF ? 0 : 0x80);
	}
...
#define READ(dest, addr)                                                              \
	do                                                                                \
	{                                                                                 \
		(dest) = mem_.read<false, false, false, callbacksActive>(addr, cycleCounter); \
		cycleCounter += 4;                                                            \
	} while (0)
#define PC_READ_OPCODE(dest)                                                      \
	do                                                                            \
	{                                                                             \
		(dest) = mem_.read<false, true, true, callbacksActive>(pc, cycleCounter); \
		pc = (pc + 1) & 0xFFFF;                                                   \
		cycleCounter += 4;                                                        \
	} while (0)
#define PC_READ_OPERAND(dest, operand)                                             \
	do                                                                             \
	{                                                                              \
		(dest) = mem_.read<false, true, false, callbacksActive>(pc, cycleCounter); \
		pc = (pc + 1) & 0xFFFF;                                                    \
		cycleCounter += 4;                                                         \
		if (callbacksActive)                                                       \
			(operand) = (dest);                                                    \
	} while (0)
#define FF_READ(dest, addr)                                         \
	do                                                              \
	{                                                               \
		(dest) = mem_.ff_read<callbacksActive>(addr, cycleCounter); \
		cycleCounter += 4;                                          \
	} while (0)
...
#define WRITE(addr, data)                                             \
	do                                                                \
	{                                                                 \
		mem_.write<false, callbacksActive>(addr, data, cycleCounter); \
		cycleCounter += 4;                                            \
	} while (0)
#define FF_WRITE(addr, data)                                      \
	do                                                            \
	{                                                             \
		mem_.ff_write<callbacksActive>(addr, data, cycleCounter); \
		cycleCounter += 4;                                        \
	} while (0)
...
#define ld_rr_nn(r1, operandLow, r2, operandHigh) \
	do                                            \
	{                                             \
	...
	void CPU::process(unsigned long const cycles)
	{
		...
		while (mem_.isActive())
		{
			...
			if (mem_.halted())
			{
				...
				while (cycleCounter < mem_.nextEventTime())
				{
				...
				dispatch:
					...
					case 0x21:
						ld_rr_nn(h, operandLow, l, operandHigh);
						break;
					...
					case 0xDD:
						...
						if (romAddress == -1)
						{
							MessageBoxA(NULL, "Invalid ROM address for 0xDD", "Fatal error", MB_OK | MB_ICONERROR);
							std::abort(); // fail hard
						}
						...
						{
							char msg[128];
							sprintf_s(msg, "No custom instruction callback found for ROM address 0x%X", romAddress);
							MessageBoxA(NULL, msg, "Missing callback", MB_OK | MB_ICONERROR);
							std::abort(); // intentional crash
						}
						...
						if (!instr->callback)
						{
							MessageBoxA(NULL, "Callback is null in CustomInstruction!", "Invalid callback", MB_OK | MB_ICONERROR);
							std::abort();
						}

						instr->callback(romAddress, instr->userData);
						goto dispatch; // Execute the normal instruction that 0xDD has replaced
					case 0xDE:
					{
						unsigned data;
						PC_READ_OPERAND(data, operandHigh);
